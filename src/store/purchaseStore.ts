...
interface PurchaseState {
    ...
    isLoading: boolean;
    error: string | null;
    fetchPurchases: (page?: number, pageSize?: number, startDate?: string, endDate?: string) => Promise<void>;
    addPurchase: (purchase: Omit<Purchase, 'id' | 'created_at'> & { id?: string }) => Promise<string>;
...
export const usePurchaseStore = create<PurchaseState>()(
    (set, get) => ({
        ...
        fetchPurchases: async (page = 1, pageSize = 20, startDate?: string, endDate?: string) => {
            ...
            try {
                ...
                const mappedPurchases = data.map((p: any) => ({
                    ...
                    items: p.purchase_items ? p.purchase_items.map((item: any) => ({
                ...
                set({
                    purchases: mappedPurchases as Purchase[],
                    isLoading: false,
                    totalCount: count || 0,
                    currentPage: page,
                    pageSize
        ...
        addPurchase: async (purchase) => {
            ...
            try {
                ...
                const purchaseDate = purchase.date ? new Date(purchase.date).toISOString() : new Date().toISOString();
