...
interface SettingsState {
    ...
    addSubCompany: (subCompany: SubCompany) => Promise<void>;
    ...
    (set, get) => ({
        ...
        fetchSettings: async () => {
            ...
            try {
                ...
                if (mainError && mainError.code !== 'PGRST116') { // PGRST116 is "Row not found"
                    console.error('Error fetching company settings:', mainError);
                    // Don't set error here, just rely on local defaults if fetch fails or is empty
                }
                ...
                set({
                    companySettings: mainSettings ? {
                        ...
                        gstNo: mainSettings.gst_no // Map snake_case to camelCase
                    } : get().companySettings,
                    subCompanies: subs ? subs.map((sub: any) => ({
                        ...
                        email: sub.email,
                        website: sub.website,
                        gstNo: sub.gst_no // Map snake_case to camelCase
                    })) : []
                });
            } catch (err: any) {
        ...
        updateCompanySettings: async (settings) => {
            ...
            try {
                ...
                // Check if a main record exists first
        ...
        addSubCompany: async (subCompany) => {
            set({ loading: true, error: null });
            try {
                // Remove the temporary ID if it's generated on frontend before insert, 
                // or let Supabase generate it and we fetch it back.
                // The frontend sends `id` for optimistic updates, but for DB we need to exclude it 
                // if it's not a valid UUID or if we want DB to generate it. 
                // However, to keep it simple and consistent with local state, let's let DB generate ID 
                // and we refetch or use returned data.
