...
export const useOrderStore = create<OrderState>((set, get) => ({
        ...
        try {
            ...
            const { data: orderData, error: orderError } = await supabase
                ...
                .insert({
                    ...
                    customer_id: order.customer_id, // Ensure this is passed
            ...
            const itemsToInsert = order.items.map(item => ({
                ...
                price: item.sellingPrice ?? item.unitPrice, // Fix: DB requires 'price' column
                description: item.description
            }));
            ...
            // Note: Stock levels (regular quantity) are automatically updated via a database trigger 
            // (tr_order_items_stock). Manual deduction here was causing double-counting.
            // Stock reversal on delete is still handled manually in deleteOrder.
            ...
            if (order.customer_id) {
                ...
                // If RPC doesn't exist, we'll fall back to simple update (riskier for concurrency but works for now)
                if (balanceError) {
    ...
    updateOrder: async (id: string, updates: Partial<Order>) => {
        ...
        try {
            ...
            // If items are being updated, that's complex (delete and re-insert or diff). 
            // For now, assuming updateOrder is mostly for status/payment updates.
            // If full edit is needed, we usually delete old items and insert new ones.

            if (Object.keys(dbUpdates).length > 0) {
                // 1. If amountPaid is being updated, fetch old value for balance sync
                let balanceChange = 0;
                let customerId = "";
    ...
    deleteOrder: async (id: string) => {
        set({ isLoading: true, error: null });
        try {
            // 1. Fetch order details to get customer and amount, plus items for stock reversal
            ...
            if (orderToDelete && orderToDelete.order_items) {
                for (const item of (orderToDelete.order_items as any)) {
                    ...
                    if (stockError) {
                        console.warn("Stock reversal RPC failed, falling back to manual update:", stockError);
    ...
    allocateCustomerPayment: async (customerId: string, amount: number) => {
        set({ isLoading: true, error: null });
        try {
            // 1. Fetch outstanding orders for this customer (Unpaid or Partial)
            ...
            if (orders && orders.length > 0) {
                for (const order of orders) {
                    ...
                    // We call updateOrder internally to handle the database update 
                    // and the reactive customer balance sync logic already inside updateOrder.
            ...
            // 2. If there's still money left (payment > all debts), 
            // update the customer balance directly as extra credit.
            if (remaining > 0) {
                const { error: balanceError } = await supabase.rpc('update_customer_balance', {
                    p_customer_id: customerId,
                    p_amount: remaining
        ...
        const subscription = supabase
            ...
            .on('postgres_changes', { event: '*', schema: 'public', table: 'orders' }, async (payload) => {
                ...
                if (payload.eventType === 'INSERT') {
                    // Always re-fetch the first page on insert to keep it fresh
                    if (currentPage === 1) {
                        get().fetchOrders(1, pageSize);
                    } else {
                        // Just update the total count if we're not on the first page
                        set(state => ({ totalCount: state.totalCount + 1 }));
                    }
                } else if (payload.eventType === 'UPDATE') {
                    // Update the item if it exists in the current page
                    const updatedOrder = payload.new;
                    set(state => ({
                        orders: state.orders.map(o => o.id === updatedOrder.id ? { ...o, ...updatedOrder } : o)
                    }));
                } else if (payload.eventType === 'DELETE') {
                    // Remove the item if it exists in the current page
                    const deletedId = payload.old.id;
                    if (orders.some(o => o.id === deletedId)) {
                        set(state => ({
                            orders: state.orders.filter(o => o.id !== deletedId),
                            totalCount: Math.max(0, state.totalCount - 1)
                        }));
                    } else {
                        set(state => ({ totalCount: Math.max(0, state.totalCount - 1) }));
                    }
                }
            })
            .subscribe();
